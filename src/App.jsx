...
import { 
  ...
  MapPin, Mail, Instagram, Facebook, CheckCircle2 
} from 'lucide-react';
    ...
    <nav className="fixed w-full bg-white/90 backdrop-blur-md z-50 border-b border-stone-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
          {/* LOGO DESIGN (Based on "Beech wala" - A with Leaf) */}
          <Link to="/" className="flex items-center gap-3 group">
             <div className="relative w-12 h-12 flex items-center justify-center">
                {/* Simulated Logo Graphic */}
                ...
                <Leaf className="text-ayur-green w-8 h-8 relative z-10 transform group-hover:rotate-12 transition-transform duration-500" />
                <span className="absolute -bottom-1 -right-1 text-xs font-bold bg-stone-800 text-white px-1 rounded">Dr.</span>
             </div>
             <div>
                ...
                <p className="text-[10px] text-stone-500 tracking-[0.2em] uppercase mt-0.5">Holistic Healing</p>
             </div>
          </Link>
            ...
            <div className="p-4 space-y-4">
              {navs.map((nav) => (
                <Link 
                  key={nav.name} 
                  to={nav.path} 
                  className="block text-lg font-medium text-stone-700 py-2 border-b border-stone-100"
    ...
    <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-3 gap-12">
      ...
      <div>
        ...
        <ul className="space-y-2 text-sm">
          <li><Link to="/" className="hover:text-ayur-green transition">Home</Link></li>
          <li><Link to="/services" className="hover:text-ayur-green transition">Treatments</Link></li>
          <li><Link to="/about" className="hover:text-ayur-green transition">About Dr. Abhay</Link></li>
        </ul>
      </div>
      <div>
  ...
  return (
    <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit">
      ...
      <section className="relative h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 w-full text-white pt-20">
          <motion.div 
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
               <Link to="/contact" className="bg-ayur-green text-white px-8 py-4 rounded-lg font-bold hover:bg-green-700 transition flex items-center gap-2">
                 Book Appointment <ArrowRight className="w-4 h-4"/>
               </Link>
               <Link to="/services" className="border border-white text-white px-8 py-4 rounded-lg font-bold hover:bg-white hover:text-stone-900 transition">
                 View Treatments
               </Link>
            </div>
          </motion.div>
      ...
      {/* FEATURE STRIP */}
      <div className="bg-ayur-green py-8">
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap justify-around text-white text-center gap-4">
           <div className="flex flex-col items-center">
             <span className="text-3xl font-bold">100%</span>
             <span className="text-sm opacity-80">Natural Herbs</span>
           </div>
           <div className="bg-white/20 w-px h-12 hidden md:block"></div>
           <div className="flex flex-col items-center">
             <span className="text-3xl font-bold">2025</span>
             <span className="text-sm opacity-80">Latest Research</span>
           </div>
           <div className="bg-white/20 w-px h-12 hidden md:block"></div>
           <div className="flex flex-col items-center">
             <span className="text-3xl font-bold">Fast</span>
             <span className="text-sm opacity-80">Recovery Rate</span>
           </div>
        </div>
      </div>

      {/* SHORT INTRO */}
      <section className="py-24 bg-ayur-cream">
...
const Services = () => (
  <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="pt-32 pb-20">
    <div className="max-w-7xl mx-auto px-4">
      <div className="text-center mb-16">
        <h1 className="text-4xl font-serif font-bold text-stone-800">Our Specializations</h1>
        <p className="text-stone-500 mt-2">Targeted treatments for complex conditions</p>
      </div>
      ...
      <div className="space-y-20">
        {servicesData.map((service, index) => (
          ...
          >
            {/* Image with Shutter Effect */}
            <div className="w-full md:w-1/2 group overflow-hidden rounded-2xl shadow-2xl relative">
              <div className="absolute inset-0 bg-stone-900/20 group-hover:bg-transparent transition-colors duration-500 z-10"></div>
              <img 
                src={service.image} 
                alt={service.title} 
                className="w-full h-[400px] object-cover transform group-hover:scale-110 transition-transform duration-700 ease-in-out"
              />
            </div>
            ...
            <div className="w-full md:w-1/2 space-y-6">
              <div className="flex flex-wrap gap-2">
                {service.tags.map(tag => (
                  <span key={tag} className="text-xs font-bold text-ayur-green bg-green-50 px-2 py-1 rounded uppercase tracking-wider">
                    {tag}
                  </span>
                ))}
              </div>
              <h2 className="text-3xl font-serif font-bold text-stone-900">{service.title}</h2>
              <p className="text-lg text-stone-600 leading-relaxed">{service.desc}</p>
...
const About = () => (
  <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="pt-32 pb-20 bg-stone-50">
    <div className="max-w-6xl mx-auto px-4">
      <div className="bg-white p-8 md:p-16 rounded-3xl shadow-xl flex flex-col md:flex-row items-start gap-12">
         <div className="w-full md:w-1/3 relative">
            <div className="aspect-[3/4] bg-stone-200 rounded-xl overflow-hidden relative group">
         ...
         <div className="w-full md:w-2/3 space-y-6">
            <h1 className="text-4xl font-serif font-bold text-stone-800">Dr. Abhay Pratap Singh</h1>
            <p className="text-xl text-ayur-green font-medium">B.A.M.S (Ayurvedacharya) - 2025</p>
  ...
  <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="pt-32 pb-20">
    <div className="max-w-5xl mx-auto px-4">
      ...
      <div className="grid md:grid-cols-2 gap-12">
         {/* Contact Info */}
         <div className="space-y-8 p-8 bg-ayur-green text-white rounded-2xl shadow-xl h-fit">
            <div>
               <h3 className="text-xl font-bold mb-2">Clinic Location</h3>
               <p className="flex items-start gap-3 opacity-90">
            ...
            <div>
               ...
               <p className="flex items-center gap-3 opacity-90">
                  <Mail className="w-5 h-5"/> contact@abhayayulife.com
               </p>
            </div>
         </div>
         ...
         <div className="bg-white p-8 rounded-2xl shadow-lg border border-stone-100">
            <form className="space-y-6">
               ...
               <div>
                  ...
                  <textarea className="w-full px-4 py-3 rounded-lg bg-stone-50 border border-stone-200 focus:outline-none focus:border-ayur-green transition h-32" placeholder="Describe your issue"></textarea>
               </div>
               <button className="w-full bg-stone-800 text-white font-bold py-4 rounded-lg hover:bg-ayur-green transition duration-300">
                  Send Message
               </button>
            </form>
         </div>
